...
# Amendments the binary knows but does not support yet. Listing one in [features] makes the
# transactors treat it as active during transaction processing, which is the only way to reach
# Supported::No code paths on a standalone node. Empty by default: add a name only when you
# actually want to exercise it.
EXTRA_FEATURES=""

set -euo pipefail

TAG="${1:?usage: generate-amendments.sh <rippled-ref> [target-cfg]}"
CFG="${2:-$(cd "$(dirname "$0")/../configs" && pwd)/rippled-standalone.cfg}"
if [ ! -f "$CFG" ]; then
